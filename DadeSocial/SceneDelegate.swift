...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let isLoginEmailInfo = UserDefaults.standard.value(forKey: "isLoginEmail") as? Bool {
            isLoginEmail = isLoginEmailInfo
        } else {
            isLoginEmail = true
        }
        
            if LoginVal == "yes"{
//                let getLocation = Config().AppUserDefaults.object(forKey: "SetLocation") as? String
//                if getLocation == "Yes"{
