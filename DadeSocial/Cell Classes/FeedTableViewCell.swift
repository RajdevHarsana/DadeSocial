...
//        post!.liked = true
//    }
    func configure(imageUrl:[String],videoUrl:[String],type:[String]){
        self.videoArray = videoUrl
        self.videoImgArray = imageUrl
        self.type = type
        imageCollectionView.reloadData()
    }
...
//        cell.configure(image: self.videoImgArray[indexPath.item])
        let UrlType = self.type[indexPath.item]
        if UrlType == "VIDEO"{
            cell.configureCell(imageUrl: videoImgArray[indexPath.row], description: "", videoUrl: videoArray[indexPath.row])
        }else{
            cell.configureCell(imageUrl: videoArray[indexPath.row], description: "", videoUrl: nil)
        }
